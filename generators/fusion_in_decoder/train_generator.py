...
def train(args, tokenizer, model, optimizer, scheduler, train_dataset, valid_dataset, train_collator, val_collator, step, best_valid_em, checkpoint_path):
    ...
    #scaler = torch.cuda.amp.GradScaler(growth_interval=100) #amp1
    #min_scale = 128 #amp_
    ...
    with tqdm(desc='[Train]') as pbar:
        while step < args.total_steps:
            ...
            for i, batch in enumerate(train_dataloader):
                ...
                #print("target_ids:",len(target_ids[0]),target_ids)
                #print("passage_ids:",len(passage_ids[0]),len(passage_ids[0][0]),passage_ids) #ここに位置encoding入れる

                #skipped_updates  = 0 #amp_
                #with torch.cuda.amp.autocast(enabled=True): #amp2
                outputs = model(
                ...
                train_loss.backward()
                    #scaler.scale(train_loss).backward() #amp3
                    #scaler.unscale_(optimizer) #amp_

                if step % args.accumulation_steps == 0: #fid.options.pyに1って書いてた
                    torch.nn.utils.clip_grad_norm_(model.parameters(), args.clip)
                    #scaler.step(optimizer) #amp4
                    optimizer.step()
                ...
                if step % args.eval_step == 0:
                    ...
                    #valid_em = evaluate(args, valid_dataset, val_collator, tokenizer, model)
                    model.train()
                    if args.is_main:
                        ...
                        # logを記入
                        log_path_str ="log/"+str(args.name)+".txt"
                        log_path = Path(log_path_str)
                        is_file = os.path.isfile(log_path)
                        if is_file==False:
                            log_path.touch(exist_ok=True) #parentsは中間ディレクトリ
                        log_list=['step:',step,',loss:',loss]
                ...
                if args.is_main and step % args.save_freq == 0:
                    util.save(model, optimizer, scheduler, step, best_valid_em, args, checkpoint_path, f"step-{step}") #最後がname
                    #### モデルのsave model/fusion-in-decoder  'step-1'
...
if __name__ == "__main__":
    ...
    checkpoint_path = Path(args.checkpoint_dir) / args.name #つまりmodel/fusion-in-decoder
    checkpoint_path.mkdir(parents=True, exist_ok=True)
    model_path = args.model_name_or_path or (checkpoint_path / "checkpoint" / "latest") #つまり"sonoisa/t5-base-japanese"
    ...
    #load data
    train_collator = Collator(tokenizer, 256, answer_maxlength=args.answer_maxlength)
    val_collator = Collator(tokenizer, 256, answer_maxlength=args.answer_maxlength)
