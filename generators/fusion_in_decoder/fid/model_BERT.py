    ...
    def forward(self, input_ids=None, attention_mask=None, **kwargs,):
        ...
        def PE(position, d_hid, device):
            ...
            if position % 2 == 0:
                ...
                PE = np.cos(a)  # dim 2i+1
            return torch.LongTensor(PE).to(device) #.unsqueeze(0)
        ...
        position_embeddings = torch.zeros([bsz*self.n_passages, passage_length, 768], dtype=torch.long, device=input_ids.device)  
        ...
        position_embeddings = position_embeddings.reshape(1, self.n_passages*passage_length, 768)   
