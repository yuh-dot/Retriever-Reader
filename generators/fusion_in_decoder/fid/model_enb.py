...
class FiDT5(T5ForConditionalGeneration):
    ...
    def forward(self, input_ids=None, attention_mask=None, **kwargs):
            ...
            #input_ids = input_ids.view(input_ids.size(0), -1) #
            #print("input_ids2",input_ids.size())
            #print("input_ids:",len(input_ids),input_ids)    ########
        self.encoder.n_passages = input_ids.size(1)
        if attention_mask != None:
            #print(attention_mask[0][0])
            list1 = np.array([])
            list2 = np.array([], dtype=bool)
            n_pass = len(attention_mask[0])
            for i in range(n_pass):
                print("input_ids1",input_ids.size())
                print("attention_mask",attention_mask.size())
                p_len=int((attention_mask[0][i] == True).sum())
                input_ids_p = input_ids[0][i][:p_len]
                attention_mask_p = attention_mask[0][i][:p_len]
                input_ids_list = input_ids_p.to('cpu').detach().numpy().copy()
                attention_mask_list = attention_mask_p.to('cpu').detach().numpy().copy()
                #print("attention_mask_list", attention_mask_list)
                #print("input_ids_list",input_ids_list)
                for j in range(p_len):
                    list1=np.append(list1, input_ids_list[j])
                    list2=np.append(list2, attention_mask_list[j])
            input_ids2 = torch.from_numpy(list1.astype(np.float32)).clone().unsqueeze(dim=0)
            attention_mask2 = torch.from_numpy(list2.astype(np.bool8)).clone().unsqueeze(dim=0)
            print("input_ids2", input_ids2.size(), input_ids2)
            print("attention_mask2", attention_mask2.size(), attention_mask2)
            #attention_mask = attention_mask.view(attention_mask.size(0), -1)
...
class EncoderWrapper(torch.nn.Module):
    ...
    def forward(self, input_ids=None, attention_mask=None, **kwargs,):
        ...
        for i in range(len(token_type_ids)):                                                   #####
            token_type_ids[i] += 1*i                                                            #####
        #print("token_type_ids:",token_type_ids.size(),token_type_ids)
        position_embeddings = self.token_type_embeddings(token_type_ids) #position_embedding
        #print("passage_length:",passage_length)
        #print("position_embeddings:",position_embeddings.size(),position_embeddings)                          
        position_embeddings = position_embeddings.reshape(1, 50*passage_length, 768)             #####
        #print("position_embeddings:",position_embeddings.size(),position_embeddings)                          ####################
