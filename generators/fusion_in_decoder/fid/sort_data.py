...
@curry
def set_data(data, n_context, mistake_lists, global_rank=-1, world_size=-1):
    examples = load_data(data, global_rank=global_rank, world_size=world_size)
    #print(examples[0])
    return Dataset(examples, n_context, mistake_lists)
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        self.mistake_lists=mistake_lists
    ...
    def sort_data_moto(self):
        if self.n_context is None or not "score" in self.data[0]["ctxs"][0]:
            return
        for ex in self.data:
            ex["ctxs"].sort(key=lambda x: float(x["score"]), reverse=True) #keyはxの"score"高いのでsortするということ、reverse=Trueは降順
            num=0
            for i in range(self.n_context):
                if(ex["ctxs"][i]["has_answer"]==True):
                    ex["ctxs"].insert(num,ex["ctxs"][i])
                    del ex["ctxs"][i+1]
                    num+=1
                    print("問題",ex['id'],"の記事",i)
                    # # logを記入
                    # log_path_str ="log.txt"
                    # log_path = Path(log_path_str)
                    # is_file = os.path.isfile(log_path)
                    # if is_file==False:
                    #     log_path.touch(exist_ok=True) #parentsは中間ディレクトリ
                    # log_list=["問題",ex['id'],"の記事",i]
        ...
        for i in self.mistake_lists:
            ...
            for i in range(self.n_context):
                if(ex["ctxs"][i]["has_answer"]==True):
                    ...
                    print("問題",ex['id'],"の記事",i)
                    # logを記入
                    log_path_str ="log_kizyun_has_ans.txt"
                    log_path = Path(log_path_str)
                    is_file = os.path.isfile(log_path)
                    if is_file==False:
                        log_path.touch(exist_ok=True) #parentsは中間ディレクトリ
            ...
            # if num==0:
            #     print("has_answer=trueなし問題：",ex['id'])
